...
export default function KYC() {
  ...
  const [isSyncing, setIsSyncing] = useState(true) // ─── THE BARRIER ───
  ...
  // FORCE SYNC ON MOUNT: Prevents jumping to Step 1 if DB says user is pending
  useEffect(() => {
    refreshUser().finally(() => setIsSyncing(false))
  }, [])

  const videoRef = useRef(null)
  const [stream, setStream] = useState(null)

  // Wait for server verification so we don't show the wrong screen
  ...
  // ─── STATUS CHECK (Always uses the freshest data from refreshUser) ───
  const isPending = 
      ...
      {step === 1 && (
        <div className="space-y-6 animate-slide-up">
          ...
          <div className="space-y-5 pt-4">
            <input className="field-input" placeholder='Full Legal Name' value={form.fullName} onChange={e => setForm({...form, fullName: e.target.value})} />
            ...
            <input className="field-input" placeholder='ID Number' value={form.docNumber} onChange={e => setForm({...form, docNumber: e.target.value})} />
            <textarea className="field-input h-28" placeholder='Residential Address' value={form.address} onChange={e => setForm({...form, address: e.target.value})} />
          </div>
      ...
      {step === 2 && (
        <div className="space-y-8 animate-slide-up">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <label className="cursor-pointer border-2 border-dashed border-noir-400 p-8 rounded-3xl text-center bg-noir-800/50">
        ...
        <div className="space-y-6 text-center animate-slide-up">
          ...
          <div className="pt-4 flex flex-col gap-3">
            {!stream && !form.selfieImage ? (
              ...
              <button className="btn-ghost w-full py-4 text-gold border border-gold/20 rounded-xl" onClick={startCamera}>RETAKE PHOTO</button>
            )}
          </div>
